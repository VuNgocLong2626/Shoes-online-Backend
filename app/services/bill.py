...
from app.db.repositories.product_detail.get_all_john_image import get_all_john_image
...
class BillServices():
    ...
    def create_bill(bill_in: _bill_schemas.BillCreate):
        ...
        for detail in bill_details:
            ...
            size_quantity = get_quantity_by_id_size_quantity(detail.id_size_quantity).SizeQuantity
            quantity = size_quantity.quantity - detail.quantily
            quantity_sold = size_quantity.quantity_sold + detail.quantily
            size_quantity_in = _size_quatity_schemas.SizeQuantityUpdateQuantity(**{
            ...
            bill_detail_in = _bill_detail_schemas.BillDetailCreate(**bill_detail)
            _ = create_bill_detail(bill_detail_in)

        return bill_respon

    def get_bill(user_in: _user_schemas.UserToken, id_bill: Optional[int]=None):
        if id_bill is None:
    ...
    def update_bill(user_in: _user_schemas.UserToken, bill_in: _bill_schemas.BillUpdate):
